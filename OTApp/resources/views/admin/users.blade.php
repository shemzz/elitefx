						...
						<div class="col shadow card p-4 bg-{{Auth('admin')->User()->dashboard_style}}">
							<div class="bs-example widget-shadow table-responsive" data-example-id="hoverable-table"> 
								...
								<form class="form-inline" role="form" method="post" action="{{action('Admin\HomeController@search')}}">
									...
									<input class="shadow-sm bg-light text-dark form-control" placeholder="Search user" type="text" name="searchItem" required>
									<input type="hidden" name="_token"  value="{{ csrf_token() }}">
									<input type="submit" class="btn btn-{{$text}} m-2" value="Go">
								</form> --}}
								
								
								<table id="ShipTable" class="table text-{{$text}}"> 
									<thead> 
										<tr> 
											<th>Name</th> 
											<th>Balances</th> 
									...
									<tbody> 
										...
										<tr> 
											...
											<td>
												<a class="btn btn-primary btn-sm m-1" href="{{ url('admin/dashboard/manageuser') }}/{{$list->id}}">Manage User</a> 
											</td> 
											<td>{{$list->status}}</td> 
											<td>
											...
											<td>{{\Carbon\Carbon::#block#blockparse($list->created_at)->toDayDateTimeString()}}</td> 
										</tr> 
